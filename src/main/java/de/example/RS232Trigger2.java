...
public class RS232Trigger2 implements Runnable {

    private static final Logger log = LoggerFactory.getLogger(RS232Trigger2.class);
    private static SerialPort serialPort = null;
    private static final char[] HEX_ARRAY = "0123456789ABCDEF".toCharArray();

    @Override
    public void run() {
        log.info("RS232Trigger2 started");
        while (true) {
            try {
                init();
                while (true) {
 ...
 //                   serialPort.getInputStream().skip(serialPort.bytesAvailable());
  //                  serialPort.flushDataListener();
                    ...
                    int numRead = 0;
                    do {
                        numRead = serialPort.readBytes(bytes, bytes.length);
                        //Thread.sleep(10);
                    } while(numRead == 0);
